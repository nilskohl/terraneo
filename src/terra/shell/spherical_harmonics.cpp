...
SphericalHarmonicsTool::real_t SphericalHarmonicsTool::shconvert_eval(
    real_t***** sphdata,
    real_t      x,
    real_t      y,
    real_t      z,
    ...
    uint_t      kk )
{
    ...
    for ( uint_t ll = 0; ll <= lmax; ll++ )
    {
        ...
        {
            val =
                val + plm_[a] * ( csm_[0][mm] * sphdata[kk][ir][ll][mm][0] + csm_[1][mm] * sphdata[kk][ir][ll][mm][1] );
            a++;
        }
    }
...
SphericalHarmonicsTool::real_t
    SphericalHarmonicsTool::shconvert_eval( uint_t deg, int ord, real_t x, real_t y, real_t z )
{
    ...
    if ( deg > maxDegree_ )
    {
        Kokkos::abort( "deg must be <= maxDegree_ in SphericalHarmonicsTool::shconvert_eval!" );
    }

    if ( order > deg )
    {
        Kokkos::abort( "order must be <= deg in SphericalHarmonicsTool::shconvert_eval!" );
    }
    ...
    uint_t      kk )
{
    ...
    for ( uint_t ll = 0; ll <= lmax; ++ll )
    {
        for ( uint_t mm = 0; mm <= ll; ++mm )
        {
            ...
            vUpp += plm_[idx] *
                    ( csm_[0][mm] * sphdata[kk][ir + 1][ll][mm][0] + csm_[1][mm] * sphdata[kk][ir + 1][ll][mm][1] );
            idx++;
        }
    }
    ...
    {
        std::cerr << "Warning: shconvert_eval detect NaN or Inf. Replacing value by zero." << std::endl;
        return 0.0;
    }
#endif
...
void SphericalHarmonicsTool::plmbar( real_t* p, uint_t lmax, real_t z )
{
    ...
    {
        Kokkos::abort( "Parameter inconsistency in SphericalHarmonicsTool::plmbar!" );
    }
...
void SphericalHarmonicsTool::dplmbar( real_t* dplm, real_t* plm, uint_t lmax )
{
    if ( lmax > maxDegree_ )
    {
        Kokkos::abort( "Parameter inconsistency in SphericalHarmonicsTool::dplmbar!" );
    }
...
void SphericalHarmonicsTool::dplmbar( real_t* dplm, uint_t lmax, real_t z )
{
    if ( lmax <= 0 )
    {
        Kokkos::abort( "lmax <= 0 in SphericalHarmonicsTool::dplmbar!" );
    }

    if ( lmax > maxDegree_ )
    {
        Kokkos::abort( "lmax > maxDegree_ in SphericalHarmonicsTool::dplmbar!" );
    }
...
SphericalHarmonicsTool::real_t
    SphericalHarmonicsTool::evalVSH( uint_t deg, int ord, real_t x, real_t y, real_t z, uint_t ind, uint_t comp )
{
    if ( ind > 2 || comp > 2 || deg > maxDegree_ || uint_c( std::abs( ord ) ) > deg )
    {
        Kokkos::abort( "Parameter inconsistency in SphericalHarmonicsTool::evalVSH!" );
    }
    ...
    {
        Kokkos::abort( "Problem with components!" );
    }
    ...
    if ( std::isnan( retVal ) )
    {
        Kokkos::abort( "NaN problem with vector spherical harmonics!" );
    }
