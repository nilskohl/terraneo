...
concept OperatorLike =
    ...
    requires(
        ...
        {
            self.apply_impl( src, dst, level, operator_apply_mode, operator_communication_mode )
        } -> std::same_as< void >;
    };
...
{
    A.apply_impl( src, dst, level, operator_apply_mode, operator_communication_mode );
}
