...
class EpsilonDivDivKerngen
{
  ...
  public:
    ...
    {
        /*const int D1 = ( src_.extent( 1 ) - 1 );
        const int D2 = ( src_.extent( 2 ) - 1 );
        const int D3 = ( src_.extent( 3 ) - 1 );

        int local_subdomain_id = idx / ( D1 * D2 * D3 );
        int r                  = idx % ( D1 * D2 * D3 );
        int x_cell             = r / ( D2 * D3 );
        r                      = r % ( D2 * D3 );
        int y_cell             = r / D3;
...
int cmb_shift = ((treat_boundary_ && diagonal_ == false && at_cmb_boundary != 0) ? (
   3
)
: (
   0
...
int surface_shift = ((treat_boundary_ && diagonal_ == false && at_surface_boundary != 0) ? (
   3
)
: (
   0
