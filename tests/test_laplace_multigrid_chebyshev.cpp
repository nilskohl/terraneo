...
T test( int min_level, int max_level, const std::shared_ptr< util::Table >& table, int order, int prepost_smooth )
{
    using ScalarType       = T;
    using Laplace          = fe::wedge::operators::shell::LaplaceSimple< ScalarType >;
    using Smoother         = linalg::solvers::Chebyshev< Laplace >;
    ...
    for ( int level = min_level; level <= max_level; level++ )
    {
        ...
        std::vector< VectorQ1Scalar< ScalarType > > tmps_smoother;
        for ( int i = 0; i < 2; i++ )
        {
            tmps_smoother.emplace_back(
        ...
        assign( tmps_smoother[0], 1.0 );
        apply( A_diag, tmps_smoother[0], inverse_diagonal );

        linalg::invert_entries( inverse_diagonal );

        smoothers.emplace_back( order, inverse_diagonal, tmps_smoother, prepost_smooth );
...
int run_test()
{
    ...
    for ( int order = 1; order <= 5; ++order )
    {
        ...
        auto err = test<
            ...
            fe::wedge::operators::shell::RestrictionLinear< T > >( 0, level, table, order, prepost_smooth );

        const auto time_total = timer.seconds();
        table->add_row(
            { { "tag", "time_total" }, { "level", level }, { "time_total", time_total }, { "order", order } } );

        util::logroot << "\n\nOrder: " << order << "\n\n";

        util::logroot << "err = " << err << "\n\n";
