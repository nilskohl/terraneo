...
std::pair< double, double > test( int level, const std::shared_ptr< util::Table >& table )
{
    ...
    const auto num_dofs_velocity = 3 * kernels::common::count_masked< long >( mask_data_fine, grid::NodeOwnershipFlag::OWNED );
    const auto num_dofs_pressure = kernels::common::count_masked< long >( mask_data_coarse, grid::NodeOwnershipFlag::OWNED );
    ...
    const double avg_pressure_approximation =
        kernels::common::masked_sum( w.block_2().grid_data(), w.block_2().mask_data(), grid::NodeOwnershipFlag::OWNED ) /
        num_dofs_pressure;
